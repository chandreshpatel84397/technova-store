  ...
  const submitOrder = async (event: FormEvent) => {
    ...
    try {
      ...
      if (placeOrder.fulfilled.match(resultAction)) {
        dispatch(clearCart());
        showToast("Order placed successfully! Check your history.");
        router.push(`/orders?success=1&orderId=${resultAction.payload._id}`);
      } else {
        showToast((resultAction.payload as string) || "Failed to place order");
      }
    } catch (err) {

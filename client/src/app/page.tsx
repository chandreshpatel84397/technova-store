  ...
  const trustCards = [
    {
      ...
      copy: "Mock logistics states and order-ready UI patterns.",
    },
    {
      icon: FiShield,
      title: "Customer account",
      copy: "Protected wishlist, cart, checkout, profile, and personal order history.",
    },
    {
      icon: FiCpu,
      title: "Modern stack",
      copy: "Next.js 14, Redux Toolkit, Sass Modules, Framer Motion, and TailwindCSS.",
    },
  ];
      ...
      <main>
        <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,#dbeafe,transparent_36%),linear-gradient(135deg,#f8fafc,#eef7ff)] py-20 dark:bg-[radial-gradient(circle_at_top_left,#0f3a5f,transparent_34%),linear-gradient(135deg,#020617,#0f172a)] md:py-28">
          <div className="container-shell grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
            <div>
              <MotionText className="text-sm font-black uppercase text-brand-700 dark:text-brand-100">
                Next generation storefront
              </MotionText>
              <MotionText
                className="mt-4 max-w-3xl text-5xl font-black tracking-tight md:text-7xl"
                delay={0.05}
              >
                TechNova
              </MotionText>
              <MotionText
                ...
                delay={0.1}
              >
                Premium laptops, audio, gaming, smart home, and desk gear with a
                polished shopping experience and a production-grade frontend
                architecture.
              </MotionText>
              <MotionText className="mt-8 flex flex-wrap gap-3" delay={0.15}>
                <Link href="/products">
                ...
                <Link
                  className="rounded-xl border border-slate-200 bg-white px-6 py-4 font-bold shadow-sm dark:border-slate-700 dark:bg-slate-900"
                  href="/categories"
                >
                  Browse categories
                </Link>
            ...
            <MotionImage className="rounded-[2rem] border border-white/70 bg-white/50 p-4 shadow-glass backdrop-blur dark:border-white/10 dark:bg-white/10">
              <MotionGrid className="grid gap-4 sm:grid-cols-2">
                {heroProducts.map((item) => (
                  <MotionGridItem
                    ...
                    key={item.name}
                  >
                    <div className="relative mb-5 h-32 w-full overflow-hidden rounded-2xl">
                      <Image
                        alt={item.name}
                        fill

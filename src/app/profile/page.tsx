...
export default function ProfilePage() {
  ...
  return (
    <ProtectedRoute>
      <main className="container-shell py-12 grid gap-8">
        {/* Account info */}
          ...
          {recentOrders.length === 0 ? (
            ...
            <div className="mt-6 grid gap-4">
              {recentOrders.map((order) => (
                <div
                  className="grid grid-cols-[1fr_auto] items-center gap-4 rounded-2xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40"
                  key={order.id}
                >
                  <div>
                    <p className="text-xs font-black uppercase text-brand-600">{order.id}</p>
              ...
              {orders.length > 3 && (
                <p className="text-center text-sm text-slate-500">
                  ...
                  <Link className="font-bold text-brand-600 hover:underline" href="/orders">
                    view all
                  </Link>
                </p>
              )}
